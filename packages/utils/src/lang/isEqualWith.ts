...
export function isEqualWith(
  value: unknown,
  other: unknown,
  customizer?: (objValue: unknown, othValue: unknown, indexOrKey?: number | string, object?: unknown, source?: unknown, stack?: unknown) => boolean | undefined
): boolean {
  if (customizer) {
...
function isEqual(value: unknown, other: unknown): boolean {
  ...
  for (const key of valueKeys) {
    ...
    if (!isEqual((value as Record<string, unknown>)[key], (other as Record<string, unknown>)[key])) {
      return false;
    }
  }
