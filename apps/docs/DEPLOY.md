...
3. В настройках проекта:
   - **Root Directory**: оставьте корень проекта (не указывайте `apps/docs`, так как turbo должен запускаться из корня)
   ...
   **Примечание**: Конфигурация в `vercel.json` уже содержит все необходимые настройки. Vercel автоматически применит их при подключении репозитория. Убедитесь, что Root Directory в настройках проекта установлен в корень репозитория (не в `apps/docs`), так как команда `turbo` должна запускаться из корня монорепо.
